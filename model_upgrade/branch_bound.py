...
def branch_and_bound_max_independent_set(
    neighbor_sets: list[set[int]],
    lower_bound: list[int],
    deadline: float,
) -> list[int]:
    """
    Time-limited max independent set (Tomita-style) on sparse complement graphs.

    Max IS in complement(G) equals max clique in G.
    """
    n = len(neighbor_sets)
    if n == 0:
        return []
    ...
    for v in range(n):
        if v in best_set:
            continue
        if not best_set or not (best_set & neighbor_sets[v]):
            candidate_mask |= 1 << v
    ...
    def expand(r_list: list[int], r_size: int, candidates: int) -> None:
        ...
        for i in range(len(order) - 1, -1, -1):
            ...
            next_candidates = candidates & ~masks[vertex]
            next_candidates &= ~(1 << vertex)
            if next_candidates:
                expand(r_list + [vertex], new_size, next_candidates)

            candidates &= ~(1 << vertex)

    expand(list(best), best_size, candidate_mask)
    return best
